...
G_barplo<- ggplot(genomic.forplot,aes(x=file,y=value/1000,fill=region ))+
  ...
  labs(fill = "Region",x="",y="% Region"); G_barplo
...
Ins_boxplo <- ggboxplot(qc_ins_size, "batch", "median_insert_size",
  ...
  stat_summary(fun.y=median, geom="point", shape=18, size=4);Ins_boxplo
ggsave("./results/charts/Insert_sizeForBatches_RNAseq_box.png",width=5,height=4,dpi=300)
...
GC_boxplo <- ggboxplot(qc_gc_content, "batch", "avg_gc",
  ...
  theme(legend.position = "none")+
  stat_summary(fun.y=median, geom="point", shape=18, size=4);GC_boxplo
ggsave("./results/charts/gc_content_sizeForBatches_RNAseq_box.png",width=5,height=4,dpi=300)
...
# normal 52.47682; tumor 54.81690
aggregate(qc_gc_content$avg_gc, by = list(qc_gc_content$tissue_type), FUN = sd)
# normal 3.282505; tumor 4.260443

panel.def <- ggarrange(MR_histogram,Ins_boxplo, GC_boxplo, nrow = 1, widths = c(1,1,1), 
...
dim(RUVg_normed_Counts)
saveRDS(RUVg_normed_Counts,"./Rdata/counts_RUVg_r56858c448_20220604.rds")
...
log_filtered.expr.mat.ft <- expr.mat.tpm
...
ggsave("./results/charts/fig2_0616.pdf",fig2,width=12,height=16.5,dpi=300)
