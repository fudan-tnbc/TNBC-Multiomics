...
gene_fpkm <- readRDS("./Rdata/FUSCCTNBC_new_fpkm_448x56858_220302.rds")
dim(gene_fpkm)

samples <- colnames(gene_fpkm) %>% as.data.frame()
colnames(samples) <- "sample_id"
samples$sample<- sapply(strsplit(as.character(samples$sample_id),"_"),function(x){x[1]})
samples$type<- sapply(strsplit(as.character(samples$sample_id),"_"),function(x){x[2]})
samples$type <- gsub("rep","TT",samples$type)
samples$type[which(is.na(samples$type))] <- "TT"
samples$sample_name <- paste0(samples$sample,"_",samples$type)

colnames(gene_fpkm) <- samples$sample_name
...
# pheatmap to ggplot
sex_heatmap_withbatch_fpkm = as.ggplot(sex_heatmap_withbatch)

ggsave("./results/charts/sex check total with batch information (fpkm).pdf",sex_heatmap_withbatch_fpkm,width=6,height=4,dpi=300)
...
# pheatmap to ggplot
sex_heatmap_withbatch_tpm = as.ggplot(sex_heatmap_withbatch)

ggsave("./results/charts/sex check total with batch information (tpm).pdf",sex_heatmap_withbatch_tpm,width=6,height=4,dpi=300)
...
### logRUVg_counts
logRUVg_counts <- readRDS("./Rdata/log_counts_RUVg_r56858c448_20220604.rds")
### Screening sample for lines associated with sex-checked genes
...
row.names(annotation_col) <- annotation_col$sample_name
annotation_col$sample_name <- NULL
colnames(annotation_col) <- c("Batch")
...
# pheatmap to ggplot
sex_heatmap_withbatch_logRUVg_counts = as.ggplot(sex_heatmap_withbatch)

ggsave("./results/charts/sex check total with batch information (RUVg normalized Count).pdf",sex_heatmap_withbatch_logRUVg_counts,width=6,height=4,dpi=300)

panel.abc <- ggarrange(sex_heatmap_withbatch_fpkm, sex_heatmap_withbatch_tpm, sex_heatmap_withbatch_logRUVg_counts,
                       nrow = 3, widths = c(1, 1, 1),labels = c("a","b","c"))+
             theme(plot.margin = unit(c(0.5,0.5,0,0.5), "cm"));panel.abc
ggsave("./results/charts/panel abc(HCA).pdf",panel.abc,width=8,height=8,dpi=300)
