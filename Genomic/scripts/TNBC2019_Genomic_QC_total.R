...
ggsave("./results/charts/panel bcd.pdf",panel.bcd,width=12,height=4,dpi=300)
...
panel.e <- ggarrange(Ins_vioplo, nrow = 1, labels = c("e")) +
...
ndSNPQC_histogram <- ggplot(OncoScan_CNV_QC)+
  geom_histogram(aes(x=ndSNPQC,fill=ndSNPQC_QC,color=ndSNPQC_QC),alpha=0.5,bins = 40)+
...
MAPD_histogram <- ggplot(OncoScan_CNV_QC)+
  geom_histogram(aes(x=MAPD,fill=MAPD_QC,color=MAPD_QC),alpha=0.5,bins = 40)+
  scale_fill_manual(values = c("#001E6C","#035397",'#A64B2A','#AC7D88'))+
  scale_color_manual(values = c("#001E6C","#035397",'#A64B2A','#AC7D88'))+
...
ggsave("./results/charts/fig3_0607.pdf",fig3,width=12,height=20,dpi=300)
